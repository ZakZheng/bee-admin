    ...
    <el-form ref="form" :model="form" label-width="230px">
      ...
      <el-col :span="11">  
        <el-form-item label="菜品图片：" class="img">
          <el-upload
            action="/apis/index.php?c=image&m=upload"
          ...
          <el-dialog :visible.sync="form.dialogVisible" size="tiny">
            <img width="100%" :src="form.imageId" alt="">
          </el-dialog>
          <img :src="option.imgUrl" v-if="option.imgUrl && imgFlag" style="margin-bottom: 30px;">
          <div class="cropImg" v-if="option.imgUrl && !imgFlag">
            <vue-cropper
              ref="cropper"
              :img="option.imgUrl"
      ...
      <el-col :span="22" class="button-footer"> 
        <el-form-item>
...
export default {
  ...
  watch: {
    proData(val) {
      ...
      console.log(newVal)
      newVal.tasteIds = newVal.taste_ids.split(',')
      newVal.tasteIds.forEach((item, index) => {
        newVal.tasteIds[index] *= 1
      })
      newVal.imageId = newVal.image_id
      newVal.productName = newVal.product_name
      newVal.categoryId = newVal.cate_id
      console.log(newVal)
      this.option.imgUrl = newVal.image_url
      this.form = newVal
    }
  },
  methods: {
    ...
    subImg(file, fn) {
      ...
      param.append('imageId', convertBase64UrlToBlob(file))
      axios
        .post(`/apis/index.php?c=image&m=upload`, param, config)
        .then(res => {
          if (res.data.code !== 0) {
    ...
    _getProductInfo() {
      Axios({
        ...
        .then(res => {
          if (res.data.code === 0) {
            this.proData = res.data.data.product
          } else {
            this.$message.error(res.data.msg)
          }
        })
